...
def lots(network, data, target, target_class=None, epsilon=None, stepwidth=1. / 255., iterations=1):
  ...
  and returns the adversarial image and whether it was classified as the intended target_class or has reached the
  desired epsilon.
  ...
  :param epsilon: The distance between target and deep feature representation of the adversarial. If given, LOTS will
  stop when this distance is reached.
...
def lots_(network, data, target, stepwidth):
  ...
  :param target: The targets that should be reached (this is in deep feature dimensions),
  a full batch of targets is required
